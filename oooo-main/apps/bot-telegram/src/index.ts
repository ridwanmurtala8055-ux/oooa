...
function buildHelpText() {
  return [
    ...
    '/pnlcard <position_id> — generate and send a PnL card image'
  ].join('\n');
}
...
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="675" viewBox="0 0 1200 675">
  <defs>
    <linearGradient id="bg" x1="0" y1="0" x2="1" y2="1">
      ...
      <stop offset="100%" stop-color="#152736"/>
    </linearGradient>
...
async function sendPnlCard(ctx: any, userId: string, positionId: string) {
  ...
  await writeAudit(userId, 'telegram.pnl.card.generate', { positionId, mint: p.mint, pnlUsd, pnlPct });
...
bot.command('pnlcard', async (ctx) => {
  const userId = String(ctx.from?.id || null);
  const parts = ctx.message.text.trim().split(/\s+/);
  if (parts.length < 2) return ctx.reply('Usage: /pnlcard <position_id>');
  return sendPnlCard(ctx, userId, parts[1]);
});
